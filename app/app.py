...
def export_results(session_id):
    ...
    # Header row
    headers = ['ファイル名', '評価値', '再生順', '再生回数', '回答時刻']
    ...
    for row_idx, result in enumerate(results, 2):
        # Try to store file name as numeric if possible (e.g., "001" -> 1)
        file_name_value = result.get('file_name')
        numeric_value = None
        if isinstance(file_name_value, (int, float)):
            numeric_value = file_name_value
        else:
            try:
                # Try integer
                numeric_value = int(file_name_value)
            except Exception:
                try:
                    # Try float
                    numeric_value = float(str(file_name_value))
                except Exception:
                    numeric_value = None

        if numeric_value is not None:
            ws.cell(row=row_idx, column=1).value = numeric_value
        else:
            ws.cell(row=row_idx, column=1).value = file_name_value
        ws.cell(row=row_idx, column=2).value = result['rating']
        ws.cell(row=row_idx, column=3).value = result['order']
        # Use play_count if available, fall back to repeat_count for compatibility
        ws.cell(row=row_idx, column=4).value = result.get(
            'play_count', result.get('repeat_count', 0))

        # Format timestamp
        timestamp = datetime.fromisoformat(result['timestamp'])
        ws.cell(row=row_idx, column=5).value = timestamp.strftime(
            '%Y-%m-%d %H:%M:%S')
